...
## mouse   X1   1.310e-02 1.145e-01
## mouse.1 X2   1.509e-01 3.885e-01
## mouse.2 X3   1.322e-01 3.636e-01
## mouse.3 X4   2.523e-11 5.023e-06
## mouse.4 X5   1.879e-10 1.371e-05
...
x <- replicate(100,  {
  y <- simulate(mmixed)[,1]
  m <- glm.nb(modelform, data = cbind(gene = y, mat))
  list(list(coef = coef(m), vcov = vcov(m)))
})

do.call(rbind, map(x, "coef")) %>%
  apply(2, sd)
coef(summary(mmixed))

cbind(coef(summary(mmixed)),
simvar = do.call(rbind, map(x, "vcov") %>% map(diag)) %>% sqrt %>%
  apply(2, mean)
)


##                            Estimate Std. Error   z value      Pr(>|z|)    simvar
## X1:conditionHEP|Wildtype  -7.103366  0.1256707 -56.52366  0.000000e+00 0.1070544
## X1:conditionHEP|DoubleKO  -7.926666  0.1613386 -49.13064  0.000000e+00 0.1468570
## conditionHEP|Wildtype:X2  -9.348529  0.4174393 -22.39494 4.408787e-111 0.3492303
## conditionHEP|DoubleKO:X2 -10.133175  0.5549899 -18.25830  1.777282e-74 0.5167947
## conditionHEP|Wildtype:X3  -7.981858  0.6627147 -12.04418  2.081273e-33 0.6252606
## conditionHEP|DoubleKO:X3 -10.723040  1.0239517 -10.47221  1.159020e-25 1.0123840
## conditionHEP|Wildtype:X4 -13.776912  0.6080382 -22.65797 1.164234e-113 0.6052846
